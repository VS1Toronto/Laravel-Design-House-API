...
{
    //------------------------------------------------------------------------
    //  Here we inject the Invitation Repository into this controller
    //  which then gives us access to all the methods defined in the contract 
    //  and in the base repository
    //
    ...
    public function invite(Request $request, $teamId)
    {
        ...
        if(! $user->isOwnerOfTeam($team)){
            return response()->json([
                'email' => 'You are not authorized to invite as you are not the team owner'
            ], 401);
        }
        ...
        //  If the recipient does not exist send the invitation to join the team
        //
        if(! $recipient){
